...
public class BaseDataChannel {
    ...
    public static void doServerDispatch(Object handler, String message, EntityPlayer player, ChannelInput data) {
        if (!serverDispatch(handler, message, player, data)) System.out.printf(
            "No ServerMessageHandler for '%s' found in %s\n",
            message,
            handler.getClass()
                .getName());
    }
    ...
    public static void doClientDispatch(Object handler, String message, ChannelInput data) {
        if (!clientDispatch(handler, message, data)) System.out.printf(
            "No ClientMessageHandler for '%s' found in %s\n",
            message,
            handler.getClass()
                .getName());
    }
    ...
    static class DataPacket implements ChannelOutput {
        ...
        public void close() {
            ...
            pipe.attr(FMLOutboundHandler.FML_MESSAGETARGET)
                .set(target);
            pipe.attr(FMLOutboundHandler.FML_MESSAGETARGETARGS)
                .set(arg);
            pipe.writeAndFlush(pkt)
