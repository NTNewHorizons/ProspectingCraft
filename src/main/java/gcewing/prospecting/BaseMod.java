...
import gcewing.prospecting.BaseModClient.IModel;
...
public class BaseMod<CLIENT extends BaseModClient<? extends BaseMod>> extends BaseSubsystem implements IGuiHandler {
    ...
    public BaseMod() {
        ...
        modPackage = modClass.getPackage()
            .getName();
        ...
        resourceURL = getClass().getClassLoader()
            .getResource(resourceRelDir);
        subsystems.add(this);
    ...
    public void preInit(FMLPreInitializationEvent e) {
        serverSide = e.getSide()
            .isServer();
        clientSide = e.getSide()
            .isClient();
        if (clientSide) {
            client = initClient();
            proxy = client;
    ...
    protected BaseSubsystem newSubsystem(String className) {
        try {
            return (BaseSubsystem) Class.forName(className)
                .newInstance();
        } catch (Exception exc) {
    ...
    void addTradeHandler(int villagerID, IVillageTradeHandler handler) {
        VillagerRegistry.instance()
            .registerVillageTradeHandler(villagerID, handler);
    }
    ...
    public IModel getModel(String name) {
        ...
        if (model == null) {
            if (name.endsWith(".obj")) model = ObjModel.fromResource(loc);
            else model = BaseModel.fromResource(loc);
    ...
    public static void sendTileEntityUpdate(TileEntity te) {
        ...
        if (packet != null) {
            ...
            ServerConfigurationManager cm = FMLCommonHandler.instance()
    ...
    Object createGuiElement(Class cls, EntityPlayer player, World world, BlockPos pos, int param) {
        try {
            if (debugGui) System.out
                .printf("BaseMod.createGuiElement: Looking for create method on %s for %s in %s\n", cls, player, world);
