...
import static gcewing.prospecting.Vector3.getDirectionVec;
...
public class Trans3 {
    ...
    public Vector3 ip(Vector3 u) {
        return rotation.imul(u.sub(offset))
            .mul(1.0 / scaling);
    }
    ...
    public static AxisAlignedBB boxEnclosing(Vector3 p, Vector3 q) {
        return AxisAlignedBB
            .getBoundingBox(min(p.x, q.x), min(p.y, q.y), min(p.z, q.z), max(p.x, q.x), max(p.y, q.y), max(p.z, q.z));
    }
