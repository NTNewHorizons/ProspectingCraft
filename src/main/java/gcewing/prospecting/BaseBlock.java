...
import static gcewing.prospecting.BaseMod.*;
...
public class BaseBlock<TE extends TileEntity> extends BlockContainer implements BaseMod.IBlock {
    ...
    private void dumpProperties() {
        ...
        for (int i = 0; i < numProperties; i++) {
            ...
            for (int j = 0; j < values.length; j++) System.out.printf("   %s: %s\n", j, values[j]);
        }
    }
    ...
    public int getMetaFromState(IBlockState state) {
        ...
        for (int i = numProperties - 1; i >= 0; i--) {
            ...
            if (debugState) System.out
                .printf("BaseBlock.getMetaFromState: property %s value %s --> %s of %s\n", i, value, k, values.length);
    ...
    public String getQualifiedRendererClassName() {
        ...
        if (name != null) name = getClass().getPackage()
            .getName() + "." + name;
        return name;
    }
    ...
    public TE getTileEntity(IBlockAccess world, BlockPos pos) {
        if (hasTileEntity()) {
            ...
            if (tileEntityClass.isInstance(te)) return (TE) te;
        }
        return null;
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, BlockPos pos) {
        ...
        else super.setBlockBoundsBasedOnState(world, pos.x, pos.y, pos.z);
    }
